...
class DashboardMenu:
    ...
    def show_dashboard_content(self):
        ...
        # Left: app title / logo
        tk.Label(left, text="📄  Preservation Universe", font=("Segoe UI", 20, "bold"),
    ...
    def export_selected_bids(self):
        ...
        try:
            ...
            if use_docx:
                ...
                # Title
                doc.add_heading("Preservation Universe Bid Proposal - Multiple Projects", 0)
    ...
    def show_settings(self):
        ...
        tk.Label(info_content, text="Preservation Universe", 
                font=("Arial", 12, "bold"), 
                fg=self.colors['text_primary'], 
        ...
        tk.Label(info_content, text="© 2024 Preservation Universe. All rights reserved.", 
                font=("Arial", 9), 
                fg=self.colors['text_secondary'], 
                bg=self.colors['white']).pack(anchor='w')
        
        # Updates section
        updates_section = tk.Frame(settings_container, bg=self.colors['white'], relief="solid", bd=1)
        updates_section.pack(fill='x', pady=(0, 20))
        
        updates_header = tk.Frame(updates_section, bg=self.colors['primary_blue'], height=50)
        updates_header.pack(fill='x')
        updates_header.pack_propagate(False)
        
        tk.Label(updates_header, text="Updates", 
                font=("Arial", 16, "bold"), 
                fg=self.colors['button_text'], 
                bg=self.colors['primary_blue']).pack(side='left', padx=20, pady=15)
        
        updates_content = tk.Frame(updates_section, bg=self.colors['white'])
        updates_content.pack(fill='x', padx=30, pady=20)
        ...
        try:
            ...
            version_label = tk.Label(updates_content, 
                ...
                try:
                    ...
                    if hasattr(main, 'update_manager') and main.update_manager:
                        ...
                        messagebox.showinfo("Update Check", 
                            ...
                            "Please ensure the app is properly configured.")
                except Exception as e:
                    messagebox.showerror("Update Check Error", 
                        f"Could not check for updates:\n{str(e)}")
            
            check_button = tk.Button(updates_content, 
            ...
            tk.Label(updates_content, 
                    text="Update system not available", 
                    font=("Arial", 10), 
                    fg=self.colors['text_secondary'], 
                    bg=self.colors['white']).pack(anchor='w')
